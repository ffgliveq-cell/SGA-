...
local function CP(n)
    ...
    PF.Parent = RP;PF.Size = UDim2.new(1, 0, 1, 0);PF.BackgroundTransparency = 1;PF.CanvasSize = UDim2.new(0, 0, 2.2, 0);PF.ScrollBarThickness = 4;PF.Visible = false;Pgs[n] = PF;return PF
end

local M_S = CP("M");local F_F = CP("F");local T_P = CP("T");local S_P = CP("S")
local btnY = 60

local function CT(t, p)
    local b = Instance.new("TextButton")
    b.Parent = LP;b.Size = UDim2.new(0, 150, 0, 40);b.Position = UDim2.new(0, 15, 0, btnY);b.BackgroundColor3 = Color3.fromRGB(20, 20, 25);b.Text = t;b.TextColor3 = Color3.fromRGB(255, 255, 255);b.TextSize = 13;b.Font = Enum.Font.SourceSansBold;btnY = btnY + 45;local c = UC:Clone();c.CornerRadius = UDim.new(0, 8);c.Parent = b;table.insert(Tabs, b)
    b.MouseButton1Click:Connect(function()
        for _, v in pairs(Pgs) do v.Visible = false end;p.Visible = true;for _, x in pairs(Tabs) do x.BackgroundColor3 = Color3.fromRGB(20, 20, 25) end;b.BackgroundColor3 = Color3.fromRGB(40, 120, 40)
    end)
end
...
Tabs.BackgroundColor3 = Color3.fromRGB(40, 120, 40)

local CB = Instance.new("TextButton");CB.Parent = LP;CB.Size = UDim2.new(0, 150, 0, 35);CB.Position = UDim2.new(0, 15, 1, -45);CB.BackgroundColor3 = Color3.fromRGB(150, 40, 40);CB.Text = "إغلاق السكربت";CB.TextColor3 = Color3.fromRGB(255, 255, 255);CB.TextSize = 13;CB.Font = Enum.Font.SourceSansBold;local cc = UC:Clone();cc.CornerRadius = UDim.new(0, 6);cc.Parent = CB
CB.MouseButton1Click:Connect(function() SG:Destroy() end)

local function C_T(p, t, y, cb)
    local l = Instance.new("TextLabel");l.Parent = p;l.Size = UDim2.new(0, 280, 0, 30);l.Position = UDim2.new(0, 10, 0, y);l.BackgroundTransparency = 1;l.Text = "⬅️ " .. t;l.TextColor3 = Color3.fromRGB(255, 255, 255);l.TextSize = 13;l.TextXAlignment = Enum.TextXAlignment.Left;table.insert(Lbls, l)
    local b = Instance.new("TextButton");b.Parent = p;b.Size = UDim2.new(0, 100, 0, 26);b.Position = UDim2.new(0, 320, 0, y);b.BackgroundColor3 = Color3.fromRGB(100, 40, 40);b.Text = "تعطيل ❌";b.TextColor3 = Color3.fromRGB(255, 255, 255);b.TextSize = 12;b.Font = Enum.Font.SourceSansBold;table.insert(TglB, b);local c = UC:Clone();c.CornerRadius = UDim.new(0, 6);c.Parent = b
...
local function checkE(v)
    ...
    elseif v:IsA("MeshPart") or v:IsA("SpecialMesh") then
        local nl = v.Name:lower();if nl:find("fx") or nl:find("effect") or nl:find("hit") or nl:find("slash") or nl:find("shock") then v:Destroy() end
    end
end

local function updateO(o, t, m)
    if not o or isP(o) then return end
    local n = o.Name:lower();local p = o.Parent and o.Parent.Name:lower() or "";local ty = false
    if t == "Props" then
    ...
    if ty then
        if o:IsA("MeshPart") or o:IsA("BasePart") then
            if m then
                if not Bck.S[o] then Bck.S[o] = o.Size;Bck.T[o] = o.Transparency end;o.Size = Vector3.new(0,0,0);o.Transparency = 1
            else
                if Bck.S[o] then o.Size = Bck.S[o];o.Transparency = Bck.T[o] end
            end
        elseif o:IsA("SpecialMesh") then
            if m then
                if not Bck.X[o] then Bck.X[o] = o.Scale end;o.Scale = Vector3.new(0,0,0)
            else
...
local KL = Instance.new("TextLabel");KL.Parent = T_P;KL.Size = UDim2.new(0, 240, 0, 30);KL.Position = UDim2.new(0, 10, 0, 10);KL.BackgroundTransparency = 1;KL.Text = "⬅️ زر إخفاء وإظهار اللوحة الكلي:";KL.TextColor3 = Color3.fromRGB(255, 255, 255);table.insert(Lbls, KL)
local KI = Instance.new("TextBox");KI.Parent = T_P;KI.Size = UDim2.new(0, 100, 0, 30);KI.Position = UDim2.new(0, 320, 0, 10);KI.BackgroundColor3 = Color3.fromRGB(40, 40, 50);KI.Text = "O";KI.TextColor3 = Color3.fromRGB(255, 255, 255);KI.TextSize = 14;KI.Font = Enum.Font.SourceSansBold;local kcc = UC:Clone();kcc.CornerRadius = UDim.new(0, 6);kcc.Parent = KI
